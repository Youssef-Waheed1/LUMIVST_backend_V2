...
class IncomeStatement(Base):
    ...
    revenue = Column(Float, nullable=True)  # الإيرادات
    sales = Column(Float, nullable=True)  # المبيعات (للتوافق مع الاسم القديم)
    ...
    def __repr__(self):
        return f"<IncomeStatement(symbol='{self.symbol}', year='{self.year}')>"
...
class BalanceSheet(Base):
    ...
    def __repr__(self):
        return f"<BalanceSheet(symbol='{self.symbol}', year='{self.year}')>"
...
class CashFlow(Base):
    ...
    symbol = Column(String(20), index=True, nullable=False)
    fiscal_date = Column(Date, nullable=True)
    quarter = Column(String(10), nullable=True)  # ✅ غيرنا إلى String
    year = Column(Integer, nullable=True)
